...
export default function SelectProducts({ title, collectionId, connectProducts, onAddProduct, onClose }: IProps) {
    ...
    useEffect(() => {
        const newResult = (data?.data ?? []).map(product => ({ id: product.id, title: product.title, available: product.available, image: product.image }))
