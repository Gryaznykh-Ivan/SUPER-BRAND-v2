...
export default function Media({ collectionId, images }: IProps) {
    const [selected, setSelected] = useState<IImage | null>(null)
    const [items, setItems] = useState<IImage[]>(images)

    const [uploadImages, { isSuccess: isUploadImagesSuccess, isError: isUploadImagesError, error: uploadImageError }] = useUploadCollectionImagesMutation()
    const [updateImage, { isSuccess: isUpdateImageSuccess, isError: isUpdateImageError, error: updateImageError }] = useUpdateCollectionImageMutation()
    const [removeImage, { isSuccess: isRemoveImageSuccess, isError: isRemoveImageError, error: removeImageError }] = useRemoveCollectionImageMutation()
    ...
    useEffect(() => {
        ...
        if (isUploadImagesError) {
            if (uploadImageError && "status" in uploadImageError) {
                toast.error((uploadImageError.data as IErrorResponse).message)
            } else {
    ...
    const onDragEnd = async (e: React.DragEvent) => {
        ...
        if (selected.position !== currentPosition) {
            await updateImage({ collectionId, imageId: selected.id, position: currentPosition })
        }
    ...
    const onFilesChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        await uploadImages({ collectionId, formData: formData })
    }
