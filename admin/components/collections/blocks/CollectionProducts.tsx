...
interface IProps {
    ...
    disconnectProducts: Pick<IProduct, "id">[] | undefined;
    onChange: (obj: CollectionCreateRequest | CollectionUpdateRequest) => void;
}

export default function CollectionProducts({ onChange, collectionId, connectProducts, disconnectProducts }: IProps) {
    const router = useRouter()
    const [popup, setPopup] = useState(false)

    const itemPerPage = 20

    const [getProducts, { isError, error, isFetching, data }] = useLazyGetCollectionProductsQuery()
    ...
    useEffect(() => {
        getProducts({ collectionId, ...query })
    }, [query])
    ...
    const onRemoveProduct = (item: ICollectionProduct) => {
        onChange({
            disconnectProducts: [...disconnectProducts ?? [], { id: item.id }]
        })
    }

    const onReturnProduct = (item: ICollectionProduct) => {
        const result = disconnectProducts?.filter(c => c.id !== item.id) ?? []

        onChange({
            disconnectProducts: result.length !== 0 ? result : undefined
        })
    }
                    ...
                    <>
                        {data.data.map((product, index) =>
                            <div key={product.id} className="flex items-center px-5 py-2 space-x-4 hover:bg-gray-100">
                                ...
                                {disconnectProducts?.find(c => c.id === product.id) === undefined ? (
                                    <>
                                    ...
                                    <>
                                        <div className="flex-1">
                                            <Link className="text-sm line-through" href={`/products/${product.id}`}>{product.title}</Link>
                                        </div>
                                        <div className="">
                                            <span className={`${product.available ? "bg-green-600" : "bg-gray-400"} px-2 py-1 rounded-xl text-white text-sm`}>{product.available ? "Active" : "Draft"}</span>
                                        </div>
                                        <button className="p-2 rounded-md hover:bg-gray-200" onClick={() => onReturnProduct(product)}>
                                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
