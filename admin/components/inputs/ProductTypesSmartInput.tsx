...
export default function ProductTypesSmartInput({ onChange, value, placeholder, className, id, name }: IProps) {
    const ref = useRef<HTMLInputElement>(null)
    const [focus, setFocus] = useState(false)
    const [state, setState] = useState(value ?? "")
    const debounced = useDebounce(state)

    const [suggest, { isFetching, isSuccess, data }] = useLazyProductTypesQuery()
