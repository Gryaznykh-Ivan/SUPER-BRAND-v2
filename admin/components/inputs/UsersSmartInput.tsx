...
export default function UsersSmartInput({ onChange, value, placeholder, className, id, name, region }: IProps) {
    ...
    const [getUsersBySearch, { isSuccess, isFetching, data, error }] = useLazyGetUsersBySearchQuery();

    useEffect(() => {
        if (focus) {
            getUsersBySearch({ q: debounced, limit: 5, skip: 0 })
        }
    }, [debounced, focus])
    ...
    return (
        <div className="relative">
            <input className={`w-full text-sm border-[1px] border-gray-300 rounded-md ${className}`} type="text" autoComplete="off" name={name} value={state} id={id} placeholder={placeholder} ref={ref} onChange={onInputChange} onFocus={onFocus} onBlur={onBlur} />
            {focus &&
