...
export default function SelectVariants({ title, variantId, onVariantChange, onClose, onDone }: IProps) {
    const itemPerPage = 10

    const [getProductBySearch, { isError, isFetching, data, error }] = useLazyGetVariantsBySearchQuery();

    const [state, setState] = useState<IVariantSearch[]>([])
    ...
    useEffect(() => {
        const newResult = (data?.data ?? []).map(product => ({ id: product.id, title: product.title, image: product.image, variants: product.variants }))
