...
export default function FulfillmentStatus({ status }: IProps) {
    return (
        <div className="flex justify-center items-center pl-2 py-2">
            {ReturnStatuses[status].icon}
            <span className="font-semibold ml-3 hidden sm:block">Возврат</span>
            <span className={`${ReturnStatuses[status].className} text-white px-2 py-1 rounded-md text-sm whitespace-nowrap ml-3`}>{ReturnStatuses[status].text}</span>
        </div>
    )
}
