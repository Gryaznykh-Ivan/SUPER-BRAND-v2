...
import { CollectionCreateRequest, CollectionUpdateRequest, ICollectionProduct, IErrorResponse, IOrderProduct, IProduct, IReturnProduct } from '../../../types/api';
...
import { ReturnStatus as ReturnStatuses } from '../../../types/store';
import ReturnStatus from '../cards/ReturnStatus';
...
export default function Return({ id, offers, status, hideEdit }: IProps) {
    ...
    return (
        <div className="rounded-md bg-white shadow-sm">
            <div className="p-3 border-b-[1px] flex justify-between items-center">
                ...
                {hideEdit === false && status !== 'RETURNED' &&
                    <Link href={`/orders/${router.query.orderId}/returns/${id}`} className="text-gray-800 text-sm p-2 font-medium rounded-md hover:bg-gray-100 flex items-center">
                    ...
                    <div key={offer.id} className="flex items-center px-5 py-2 space-x-4 hover:bg-gray-100">
                        ...
                        <div className="flex-1 flex items-center py-1 space-x-4 hover:bg-gray-100">
                            <div className="flex-1 text-sm py-1">
                                ...
                                <div className="text-gray-500">Причина: {offer.reason}</div>
                            </div>
