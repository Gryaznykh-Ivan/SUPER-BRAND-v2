...
import { CollectionCreateRequest, CollectionUpdateRequest, ICollectionProduct, IErrorResponse, IOffer, IOfferSearch, IProduct, OrderCreateRequest } from '../../../types/api';
...
export default function OrderProducts({ onChange, ...data }: IProps) {
    ...
    const onAddOffer = (offer: IOfferSearch) => {
        let result: IOfferSearch[]

        if (state.find(c => c.id === offer.id) !== undefined) {
            result = state.filter(c => c.id !== offer.id) ?? []
        } else {
            result = [...state, offer]
        }
        ...
        onChange({
            offers: result.map(offer => ({ id: offer.id }))
        })
    }
            ...
            <div className="divide-y-[1px] overflow-y-auto">
                ...
                {state.map((offer) =>
                    <div key={offer.id} className="flex items-center px-5 py-2 space-x-4 hover:bg-gray-100">
                        ...
                        <button className="p-2 rounded-md hover:bg-gray-200" onClick={() => onAddOffer(offer)}>
